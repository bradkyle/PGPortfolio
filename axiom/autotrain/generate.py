...
def add_packages(config, repeat=1):
    ...
    package_dir = path.realpath(__file__).replace('axiom/autotrain/generate.pyc',train_dir)\
        .replace("axiom\\autotrain\\generate.pyc", train_dir)\
                  .replace('axiom/autotrain/generate.py',train_dir)\
        .replace("axiom\\autotrain\\generate.py", train_dir)
    files = os.listdir(package_dir)
    all_subdir = [int(s) for s in files if os.path.isdir(package_dir+"/"+str(s))]
    if all_subdir:
